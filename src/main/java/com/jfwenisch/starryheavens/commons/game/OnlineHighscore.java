...
public class OnlineHighscore implements Serializable
{
	...
	public void initObject( String[] strScore ) throws Exception { 
		elementData=new String[strScore.length];
	...
	public void compare(String strScore)
	{
		...
		for (int i=0;i<elementData.length;i++)
		{
			...
			if(userName.equals(strName))
			{
				...
				{
					System.out.println("Highscore"+strName +"-"+strName +" nicht hoch genug");
				}
			}
			else
			{
		...
		if(!bFoundEntry)
		{
			...
			System.out.println("Neuer Highscore"+strName +"-"+strName +" hinzugef�gt");
